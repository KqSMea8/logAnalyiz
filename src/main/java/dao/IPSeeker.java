 ...
 *
 */
public class IPSeeker implements Serializable {
    /**
    ...
    public static final String IP_FILE = IPSeeker.class
            .getResource("./qqwry.dat").toString().substring(5);

    // 一些固定常量，比如记录长度等等
    public static final int IP_RECORD_LENGTH = 7;
    public static final byte AREA_FOLLOWED = 0x01;
    public static final byte NO_AREA = 0x2;
     ...
     * @param ip
     */
    public void readIP(int offset, byte[] ip) {
        mbb.position(offset);
        mbb.get(ip);
        byte temp = ip[0];
        ip[0] = ip[3];
     ...
     * @return
     */
    public long getMiddleOffset(long begin, long end) {
     ...
     * @return
     */
    public IPLocation getIPLocation(long offset) {
        try {
            // 跳过4字节ip
     ...
     * @return
     */
    public IPLocation getIPLocation(int offset) {
        // 跳过4字节ip
...
//        System.out.println(is.getAddress("123.12.94.13"));
//    }
}